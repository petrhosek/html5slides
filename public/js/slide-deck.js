...
SlideDeck.prototype.onDomLoaded_ = function(e) {
  ...
  //if (!Modernizr.touch) {
    this.controller = new SlideRemoteController(this);
...
SlideDeck.prototype.addEventListeners_ = function() {
  ...
  document.addEventListener('tap', this.onTap_.bind(this), false);
};
...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (Modernizr.touch && (!!!('enableTouch' in settings) ||
    ...
    hammer.ondragend = function(e) {
      // Forward keydowns to the main slides if we're the popup.
      if (self.controller && self.controller.isPopup) {
        self.controller.sendMsg({type: e.type, direction: e.direction});
      }
    ...
    hammer.ontap = function(e) {
      // Forward keydowns to the main slides if we're the popup.
      if (self.controller && self.controller.isPopup) {
        self.controller.sendMsg({type: e.type});
      }
      self.nextSlide();
    }
  }
...
SlideDeck.prototype.onTap_ = function(e) {
  this.nextSlide();
}
